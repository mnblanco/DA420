---
title: "Homework 4: Operations Management"
...
Frequency of calls by day of week

```{r echo=FALSE}
# examine frequency of calls by day of week
kable(table(call.center.data$day_of_week), col.names = c("Day of Week", "Frequency")) %>%
...
Frequency of calls in February by hour and day of week

```{r}
# check frequency of calls in February by hour and day of week
kable(with(call.center.data, table(day_of_week, call_hour))) %>%
  kable_styling(bootstrap_options = "striped", font_size = 5)
```

Frequency of calls in week by hour and day of week for first week of February 1999
...
kable(with(selected.week, table(day_of_week, call_hour))) %>%
  kable_styling(bootstrap_options = "striped", font_size = 5)
```

Wait-time ribbon plots for the days that the bank was open during the first week of February 1999. 
...
for(index.day in seq(along=day.of.week.list)) {
  ...
  wait.time.ribbon(wait.service.data = selected.day, 
                   ...
                   use.text.tagging = TRUE, wait.time.goal = 60, wait.time.max = 120,
                   plotting.min = 0, plotting.max = 250)    
  
  selected.day$over <- ifelse(selected.day$wait > 120, 1, 0)
  
  print(paste("On", this.day.of.week, "the approximate percentage that fall above the 120 seconds line is ", 
              as.character(round(mean(selected.day$over) * 100, 1)), "%"))
  
  df[idx, ] <- c(this.day.of.week, round(mean(selected.day$over) * 100, 1))
  idx <- idx + 1
}
```
...
for(index.day in seq(along=day.of.week.list)) {
  this.day.of.week <- day.of.week.list[index.day]
  plot.title <- paste(this.day.of.week,"Call Center Arrival and Service Rates")
  ...
  plot.title <- paste(this.day.of.week, "Call Center Needs and Optimal Workforce Schedule")
  ...
  cat("\n",this.day.of.week, "Hourly Operator Requirements:","\n")
  ...
  cat("\n",this.day.of.week, "Call Center Shift Constraint Matrix","\n")
  ...
  cat("\n\n",this.day.of.week,"Call Center Summary","\n\n")
  ...
  cat("\n\n",this.day.of.week,"Call Center Summary Minimum Cost Solution:",sum(ShiftCost),"\n\n")
